...
bool TextureObject::load(std::string basepath, std::string filepath)
{
	...
	if (header->m_magic != prism::tobj_header::SUPPORTED_MAGIC)
	{
		printf("Invalid magic of tobj file! \"%s\" (have: %i, expected: %i\n",
			   m_filepath.c_str(), header->m_magic, prism::tobj_header::SUPPORTED_MAGIC);
		return false;
	}
	...
	{
		prism::tobj_texture *texture = (prism::tobj_texture *)(buffer.get() + currentTextureOffset);
...
bool TextureObject::saveToMidFormats(std::string exportpath)
{
	...
	{
		if (m_mipFilter == TextureObject::NOMIPS)
		{
			file << "nomips" << SEOL;
		}
